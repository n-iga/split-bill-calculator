...
function addTotal() {
    calcNumber(elemId_totalAmount, "+", minUnit);
    generateTable();
}
...
// 加算・減算
function calcNumber(p_elemId, p_operator, p_step) {
        let l_value = p_elemId.value === "" ? 0 : parseInt(p_elemId.value)
        p_elemId.value = eval(`${l_value} ${p_operator} ${p_step}`);
    }
            ...
            row.innerHTML = `
                    ...
                    <td>
                        <div class="payment-container">                            
                            <input type="number" value="${payment}" oninput="updatePercentage()">
                        </div>
                    </td>
                    <td><input type="number" value="${percentage}" oninput="updatePayment()" readonly></td>
                    <td><input type="checkbox" class="fixed" value="${i + 1}"></td>
                    `;
                    // <div>
                    // <button id="btn_plus_2" class="btn_decrease"></button>
                    // <button id="btn_minus_2" class="btn_increase"></button>
                    // </div>
...
function calcPayment(p_totalAmount, p_numPeople, p_remainder, p_evenSplit, p_total_tmp, i) {
        ...
        switch (calcMode) {
            ...
            case "extraPay":                                     // 余りを1名が支払う
                l_payment = p_evenSplit + (p_remainder === 0 ? 0 : p_remainder);
